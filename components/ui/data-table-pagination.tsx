...
export function DataTablePagination({
  ...
  return (
    <div className="flex items-center justify-between px-6 py-4 border-t border-border bg-background/70 backdrop-blur-md rounded-b-2xl">
      ...
      <div className="flex items-center gap-3">
        <Button
          ...
          className="h-8 w-8 p-0 rounded-xl disabled:opacity-40 transition-all"
        >
          <ChevronLeft className="h-4 w-4" />
        </Button>
        
        <span className="text-xs font-semibold text-muted-foreground font-sans">
          Halaman <span className="font-mono tabular-nums">{currentPage}</span> dari <span className="font-mono tabular-nums">{totalPages}</span>
        </span>
