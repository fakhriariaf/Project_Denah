import { getUnitBusinessState, getUnitDisplayLabel } from "@/lib/unit-business-state";
...
> = {
  available:         { fill: "#DDE8D8", stroke: "#4F6F52", text: "#2D4A30", dot: "#4F6F52",  label: "Tersedia (Indent)" },
  available_ready_stock: { fill: "#3F5941", stroke: "#243525", text: "#FFFFFF", dot: "#3F5941", label: "Tersedia Siap Huni" },
  ...
  payment_pending:   { fill: "#FBE4C9", stroke: "#9A5C21", text: "#7A3D0E", dot: "#D47A2E",  label: "Menunggu Pembayaran" },
  construction:      { fill: "#E9DDF7", stroke: "#5D4382", text: "#3D2060", dot: "#7B5EA7",  label: "Pembangunan Unit Konsumen" },
  construction_ready_stock: { fill: "#4B286D", stroke: "#2C1445", text: "#FFFFFF", dot: "#4B286D", label: "Sedang Dibangun untuk Ready Stock" },
  construction_done: { fill: "#C8EFE0", stroke: "#2E7A5E", text: "#1A5240", dot: "#3DAA7E",  label: "Selesai Bangun - Siap Akad/Serah Terima" },
  sold:              { fill: "#FFD6D6", stroke: "#C0392B", text: "#8B1A1A", dot: "#E53E3E",  label: "Terjual" },
  overdue:           { fill: "#F8D4DA", stroke: "#8B3443", text: "#6A1B2A", dot: "#C04060",  label: "Terlambat" },
  cancelled:         { fill: "#E7E9E7", stroke: "#5F6861", text: "#3D4840", dot: "#7A8880",  label: "Dibatalkan" },
...
> = {
  available:         { label: "Tersedia (Indent)", dotColor: "#4F6F52",  badgeClass: "bg-[#DDE8D8] text-[#2D4A30] border-[#4F6F52]/30" },
  available_ready_stock: { label: "Tersedia Siap Huni", dotColor: "#3F5941", badgeClass: "bg-[#3F5941] text-white border-[#243525]/30" },
  ...
  payment_pending:   { label: "Menunggu Pembayaran", dotColor: "#D47A2E",  badgeClass: "bg-[#FBE4C9] text-[#7A3D0E] border-[#9A5C21]/30" },
  construction:      { label: "Pembangunan Unit Konsumen",      dotColor: "#7B5EA7",  badgeClass: "bg-[#E9DDF7] text-[#3D2060] border-[#5D4382]/30" },
  construction_ready_stock: { label: "Sedang Dibangun untuk Ready Stock", dotColor: "#4B286D", badgeClass: "bg-[#4B286D] text-white border-[#2C1445]/30" },
  construction_done: { label: "Selesai Bangun - Siap Akad/Serah Terima",   dotColor: "#3DAA7E",  badgeClass: "bg-[#C8EFE0] text-[#1A5240] border-[#2E7A5E]/30" },
  sold:              { label: "Terjual",             dotColor: "#E53E3E",  badgeClass: "bg-[#FFD6D6] text-[#8B1A1A] border-[#C0392B]/30" },
  overdue:           { label: "Terlambat",            dotColor: "#C04060",  badgeClass: "bg-[#F8D4DA] text-[#6A1B2A] border-[#8B3443]/30" },
  cancelled:         { label: "Dibatalkan",           dotColor: "#7A8880",  badgeClass: "bg-[#E7E9E7] text-[#3D4840] border-[#5F6861]/30" },
...
export function getUnitStatusLabel(status: string | null | undefined, isReadyStock: boolean = false, t?: TranslateFn) {
  ...
  return t ? t(baseKey) : getUnitDisplayLabel(status, isReadyStock);
}
...
export function getStatusBadge(status: string | null | undefined, isReadyStock: boolean = false, t?: TranslateFn) {
  ...
  const baseKey = `timeline.${key}`;
  const labelTranslated = t ? t(baseKey) : getUnitBusinessState({ status, isReadyStock }).displayLabel;
  return {
    ...base,
