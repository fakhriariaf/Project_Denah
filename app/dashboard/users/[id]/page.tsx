import { requirePermission, hasPermission, getSessionRole } from "@/server/permissions";
import { getUserProfileData } from "@/server/actions/profile";
import { ProfileShell } from "@/components/dashboard/profile-shell";
import { getRoles } from "@/server/actions/users";
...
export default async function UserDetailPage({ params }: UserDetailPageProps) {
  // 1. Gate access: requires user.read permission to view detail pages of other users
  ...
  try {
    ...
    const canUpdateEmployment = await hasPermission(activeUser.id, "employment.update");
    const canUpdateVendor = await hasPermission(activeUser.id, "vendor_profile.update");
    const canUpdateStatus = await hasPermission(activeUser.id, "account.status.update");
    const canAssignRole = await hasPermission(activeUser.id, "user.assign_role");

    permissions = {
      canUpdateBasic: canUpdateAnyBasic || (canUpdateOwnBasic && activeUser.id === targetUserId),
      canUpdateEmployment,
      canUpdateVendor,
      canUpdateStatus: canUpdateStatus && canAssignRole,
    };
  } catch (err: any) {
    errorMsg = err.message || "Gagal memuat profil pengguna.";
  }
        ...
        <CardContent className="pt-6 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-rose-700 mt-1">
              {errorMsg === "Gagal memuat profil pengguna." ? <Translate namespace="users" translationKey="error_load" /> : errorMsg}
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-[#DDE8D8]/70 via-white/95 to-[#DDE8D8]/40 border border-[#D6DED2] shadow-sage p-6">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-2xl font-black text-[#243028] tracking-tight"><Translate namespace="users" translationKey="detail_title" /></h2>
              <p className="text-sm text-[#66736A] mt-0.5"><Translate namespace="users" translationKey="detail_desc" /></p>
            </div>
          </div>
        </div>
      </div>
      ...
      <ProfileShell 
        data={data!}
        rolesList={rolesList!}
        isOwnProfile={activeUser.id === targetUserId}
