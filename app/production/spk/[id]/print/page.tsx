      ...
      <div className="print-area max-w-4xl mx-auto mt-6 bg-white border border-[#D6DED2] rounded-3xl shadow-sage p-8 md:p-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-6 border-b-2 border-[#4F6F52]/30 gap-6">
          <div className="space-y-1 md:flex-1">
            <h2 className="text-xl font-extrabold text-[#4F6F52] tracking-wide">{companyName}</h2>
            <div className="text-xs text-[#66736A] space-y-0.5">
              ...
              <p className="flex items-center gap-1.5"><Phone className="h-3.5 w-3.5 text-[#8FAF9A]" />WhatsApp: {companyPhone}</p>
              <p className="flex items-center gap-1.5"><Mail className="h-3.5 w-3.5 text-[#8FAF9A]" />Email: {companyEmail}</p>
            </div>
          </div>
          <div className="bg-[#DDE8D8]/50 border border-[#8FAF9A]/30 rounded-2xl px-4 py-2 text-right shrink-0">
            <span className="text-[10px] font-bold text-[#4F6F52] uppercase tracking-wider block">{t("production.print_spk_num")}</span>
            <span className="font-mono text-sm font-bold text-[#243028]">{spk.spkNumber}</span>
          </div>
        </div>

        {/* TITLE */}
        <div className="text-center my-8">
          <h1 className="text-2xl font-black text-[#243028] uppercase tracking-wider">
            {t("production.print_spk_title")}
          </h1>
          <p className="text-sm font-semibold text-[#4F6F52] mt-1">{spk.title}</p>
          <p className="text-xs text-[#66736A] mt-1 font-mono">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="border border-[#D6DED2] rounded-2xl p-5 bg-[#F7F8F3]/30">
            <h3 className="text-xs font-bold text-[#4F6F52] uppercase tracking-wider mb-3 border-b border-[#D6DED2]/60 pb-1.5 flex items-center gap-1.5">
              ...
              <tbody>
                {[
                  ...
                  <tr key={k} className="align-top">
                    <td className="w-24 font-semibold text-[#66736A] py-1">{k}</td>
                    <td className="w-3 text-[#A8B0AA] py-1">:</td>
                    <td className="font-bold text-[#243028] py-1">{v}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {[
                  ...
                  <tr key={k} className="align-top">
                    <td className="w-24 font-semibold text-[#66736A] py-1">{k}</td>
                    <td className="w-3 text-[#A8B0AA] py-1">:</td>
                    <td className="font-mono text-[#243028] py-1">{v}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <div className="border border-[#D6DED2] rounded-xl p-4 flex items-center gap-3">
            <Calendar className="h-4 w-4 text-[#8FAF9A] shrink-0" />
            <div>
              <p className="text-[10px] text-[#66736A] font-bold uppercase">{t("production.print_start_date")}</p>
              <p className="font-mono font-bold text-[#243028] text-sm">{spk.startDate ? formatDate(spk.startDate) : "—"}</p>
            </div>
          </div>
          <div className="border border-[#D6DED2] rounded-xl p-4 flex items-center gap-3">
            <Calendar className="h-4 w-4 text-rose-400 shrink-0" />
            <div>
              <p className="text-[10px] text-[#66736A] font-bold uppercase">{t("production.print_end_date")}</p>
              <p className="font-mono font-bold text-[#243028] text-sm">{spk.targetEndDate ? formatDate(spk.targetEndDate) : "—"}</p>
            </div>
          </div>
        </div>

        {/* URAIAN PEKERJAAN */}
        <div className="border border-[#D6DED2] rounded-2xl p-5 mb-8">
          <h3 className="text-xs font-bold text-[#4F6F52] uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Wrench className="h-4 w-4" /> {t("production.print_job_desc")}
          </h3>
          <p className="text-sm text-[#66736A] leading-relaxed">{spk.workDescription}</p>
        </div>
        ...
        {weights.length > 0 && (
          <div className="border border-[#D6DED2] rounded-2xl overflow-hidden mb-8">
            <div className="px-5 py-3 border-b border-[#D6DED2] bg-[#F7F8F3]/70">
              <h3 className="text-xs font-bold text-[#4F6F52] uppercase tracking-wider">{t("production.print_weight_title")}</h3>
            </div>
            <table className="w-full text-xs text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#D6DED2]/60">
                {weights.map((w, i) => (
                  <tr key={i}>
                    <td className="py-2.5 px-5 font-mono font-bold text-[#4F6F52]">{w.workItemCode}</td>
                    <td className="py-2.5 px-5 text-[#243028]">{w.workItemName}</td>
                    <td className="py-2.5 px-5 text-right font-mono font-bold text-[#243028]">{w.weightPct}%</td>
                  </tr>
                ))}
                <tr className="bg-[#DDE8D8]/20 font-bold">
                  <td colSpan={2} className="py-3 px-5 text-[#4F6F52] uppercase text-xs">{t("production.print_total_weight")}</td>
                  <td className="py-3 px-5 text-right font-mono text-[#4F6F52]">{weights.reduce((s, w) => s + w.weightPct, 0)}%</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        {/* RAB */}
        <div className="border border-[#D6DED2] rounded-2xl overflow-hidden mb-12">
          <div className="px-5 py-3 border-b border-[#D6DED2] bg-[#F7F8F3]/70">
            <h3 className="text-xs font-bold text-[#4F6F52] uppercase tracking-wider">{t("production.print_rab_title")}</h3>
          </div>
          <div className="px-5 py-4 flex items-center justify-between">
            <span className="font-semibold text-[#66736A]">{t("production.print_total_budget")}</span>
            <span className="font-mono font-black text-xl text-[#4F6F52]">{formatRupiah(spk.rabAmount ?? 0)}</span>
          </div>
        </div>
          ...
          {[t("production.print_sign_assignor"), t("production.print_sign_approved"), t("production.print_sign_executor")].map((title, i) => (
            <div key={i} className="space-y-16">
              <div className="space-y-1">
                <span className="text-[#66736A] block">{title}</span>
                <span className="text-[10px] text-[#A8B0AA] italic">{t("production.print_sign_note")}</span>
              </div>
              <div className="space-y-1">
                <div className="w-full border-b border-dashed border-[#66736A] mx-auto" />
                <span className="font-bold text-[#243028] block">
