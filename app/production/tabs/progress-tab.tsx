...
const SPK_STATUS_LABELS: Record<string, string> = {
  active: "Aktif", proses_konstruksi: "Proses Konstruksi", selesai_konstruksi: "Selesai Konstruksi",
  overdue: "Terlambat", completed: "Selesai", cancelled: "Batal", draft: "Draft",
};
...
export function ProgressTab({
  ...
  isSuperAdmin = false, isPengawas = false, isVendor = false,
  ...
  }, [externalProgressSpkId]);

  const handleLoadSpkWeights = async (spkId: string) => {
    try {
  ...
  const handleCompleteConstructionWithBast = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) { setSuccessMessage(`Unit "${bastUnit.code}" berhasil dinyatakan selesai pembangunan dan status berubah menjadi Tersedia - Ready Stock!`); setBastDialogOpen(false); setBastUnit(null); setBastSpk(null); setBastPdfFile(null); router.refresh(); }
  ...
  return (
    <div className="space-y-6">
      ...
      {successMessage && <div className="flex items-center gap-3 p-4 rounded-lg bg-[#DDE8D8] border border-[#8FAF9A]/40 text-[#4F6F52] text-sm font-medium animate-fade-in"><CheckCircle2 className="h-5 w-5 shrink-0" /><span>{successMessage}</span></div>}
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <Button onClick={() => setHandoverOpen(true)} className="bg-primary hover:bg-[#4F6F52] text-primary-foreground font-semibold text-xs"><Calendar className="mr-1.5 h-3.5 w-3.5" />{t("production.btn_handover_calc")}</Button>
      </div>
            ...
            return (
              <div className="space-y-6 animate-fade-in">
                <div className="pb-4 border-b border-border flex justify-between items-center">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {unit.readyStockSource === "construction_flow" && <span className="bg-[#EAF2EC] text-[#4F6F52] border border-[#8FAF9A]/30 text-[10px] font-extrabold px-2 py-0.5 rounded-lg">🏗️ Konstruksi ERP</span>}
                      {unit.readyStockSource === "legacy_ready_stock" && <span className="bg-[#F4F6F0] text-[#606C5A] border border-[#8FAF9A]/20 text-[10px] font-extrabold px-2 py-0.5 rounded-lg">🏠 Legacy Ready Stock</span>}
                      ...
                      {unit.isReadyStock ? <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-extrabold px-2 py-0.5 rounded-lg">✓ Ready Stock</span> : <span className="bg-blue-50 text-blue-700 border border-blue-200 text-[10px] font-extrabold px-2 py-0.5 rounded-lg">Indent</span>}
                    </div>
                  </div>
                  {((unit.status === "construction_done") || ((unit.status === "construction" || unit.status === "overdue") && unit.constructionProgress === 100)) && (
                    <Button size="sm" disabled={isSubmitting} onClick={() => { setBastUnit(unit); setBastSpk(spk); setBastPdfFile(null); setBastDialogOpen(true); }} className="bg-[#4F6F52] hover:bg-[#3D563F] text-white font-extrabold text-xs h-9 rounded-xl flex items-center gap-1.5 shadow-[0_4px_12px_rgba(79,111,82,0.25)] transition-all"><CheckCircle2 className="h-4 w-4 text-white" />Selesai Pembangunan</Button>
                  )}
                </div>

                {/* BAST Vendor section */}
                <div className="space-y-2">
                  <p className="text-[11px] font-extrabold uppercase tracking-wider text-[#66736A] px-1">{unit.readyStockSource === "construction_flow" ? "BAST Vendor ke Developer (Wajib)" : "BAST Vendor ke Developer (Opsional / Arsip)"}</p>
                  {activeUnitBast ? (
                    <div className="p-3.5 bg-[#4F6F52]/5 border border-[#4F6F52]/10 rounded-2xl flex items-center justify-between text-xs transition-all hover:bg-[#4F6F52]/10">
                      <div className="flex items-center gap-2.5 min-w-0"><div className="p-2 bg-[#4F6F52]/10 text-[#4F6F52] rounded-xl shrink-0"><UploadCloud className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="font-extrabold text-[#243028] text-xs truncate">Dokumen Terunggah</p><p className="text-[10px] text-[#66736A] font-mono truncate max-w-[280px] mt-0.5">{activeUnitBast.fileName}</p></div></div>
                      <a href={activeUnitBast.fileUrl} target="_blank" rel="noopener noreferrer" className="bg-[#4F6F52] hover:bg-[#3D563F] text-white font-extrabold text-xs px-3.5 py-2 rounded-xl shadow-sm transition-all hover:scale-[1.02] flex items-center gap-1.5 shrink-0 ml-2"><ExternalLink className="h-3.5 w-3.5 text-white" />Lihat PDF</a>
                    </div>
                  ) : (unit.readyStockSource === "legacy_ready_stock" || unit.readyStockSource === "manual_ready_stock") && (
                    <div className="p-3.5 bg-gray-50/50 border border-gray-200/80 rounded-2xl flex items-center justify-between text-xs"><div className="flex items-center gap-2"><div className="p-1.5 bg-gray-100 text-gray-400 rounded-lg"><FileText className="h-4 w-4" /></div><div><p className="font-bold text-[#66736A] text-xs">Arsip BAST Vendor Kosong</p><p className="text-[9px] text-muted-foreground mt-0.5">Tidak wajib untuk unit ready stock legacy.</p></div></div></div>
                  )}
                </div>
                ...
                <div className="bg-white border border-[#D6DED2] rounded-2xl p-5 shadow-[0_4px_20px_rgba(143,175,154,0.08)] space-y-4">
                  <div className="flex items-center justify-between"><h4 className="font-extrabold text-sm text-[#243028] flex items-center gap-2"><FileText className="h-4 w-4 text-[#4F6F52]" />BAST Konsumen (Developer ke Konsumen)</h4>{customerBast && customerBast.customerName && <span className="bg-[#EAF2EC] text-[#4F6F52] border border-[#8FAF9A]/30 text-[9px] font-extrabold px-2 py-0.5 rounded-lg">Terhubung KPR</span>}</div>
                  {(() => {
                    if (!customerBast || !customerBast.bookingId) return <div className="text-center py-5 border border-dashed border-[#D6DED2] rounded-xl bg-[#F7F8F3]/40"><AlertTriangle className="h-6 w-6 text-amber-500 mx-auto mb-2 animate-pulse" /><p className="text-xs font-bold text-[#243028]">Unit Belum Terjual / Booking Tidak Aktif</p><p className="text-[10px] text-[#66736A] mt-1 px-4 leading-relaxed">Unggah BAST Konsumen dinonaktifkan karena unit belum memiliki booking/penjualan yang aktif.</p></div>;
                    ...
                    return (
                      <div className="space-y-3">
                        <div className="p-3 bg-[#F7F8F3]/60 border border-[#D6DED2] rounded-xl text-xs space-y-1.5 font-sans">
                          <div className="grid grid-cols-3 gap-1"><span className="text-[#66736A]">Konsumen:</span><span className="col-span-2 font-extrabold text-[#243028]">{customerBast.customerName}</span><span className="text-[#66736A]">No. Booking:</span><span className="col-span-2 font-mono font-bold text-[#4F6F52]">{customerBast.bookingNumber}</span></div>
                        </div>
                        {!customerBast.fileName ? (
                          <div className="text-center py-4 border border-dashed border-[#8FAF9A]/40 rounded-xl bg-white space-y-2">
                            <UploadCloud className="h-6 w-6 text-[#8FAF9A] mx-auto animate-bounce" /><p className="text-xs font-bold text-[#243028]">Unggah PDF BAST Konsumen</p>
                            <div className="px-6 pt-2"><input type="file" id="customer-bast-file" accept="application/pdf" className="hidden" onChange={async (ev) => { const file = ev.target.files?.[0]; if (!file) return; setIsSubmitting(true); setErrorMessage(null); setSuccessMessage(null); try { const fd = new FormData(); fd.append("file", file); const uRes = await fetch("/api/upload-attachment", { method: "POST", body: fd }); if (!uRes.ok) throw new Error("Gagal mengunggah berkas BAST."); const fData = await uRes.json(); const res = await uploadCustomerBastFromProduction(unit.id, customerBast.bookingId, customerBast.customerId, { fileName: file.name, fileUrl: fData.url, mimeType: file.type, fileSize: file.size }); if (res.success) { setSuccessMessage("✓ Berkas BAST Konsumen berhasil diunggah dan disinkronkan ke KPR!"); await handleViewUnitProgress(unit.id); } } catch (err: any) { setErrorMessage(err.message || "Gagal mengunggah BAST."); } finally { setIsSubmitting(false); } }} /><label htmlFor="customer-bast-file" className="inline-flex items-center justify-center bg-[#4F6F52] hover:bg-[#3D563F] text-white font-extrabold text-xs px-4 py-2 rounded-xl shadow-md cursor-pointer transition-all active:scale-95 hover:scale-[1.02] gap-1.5"><UploadCloud className="h-4 w-4" />Pilih Berkas PDF BAST</label></div>
                          </div>
                        ) : (
                          <div className="space-y-3">
                            <div className={`p-3 border rounded-xl flex items-center justify-between text-xs ${docStatus === "verified" ? "bg-emerald-50/50 border-emerald-200" : docStatus === "rejected" ? "bg-rose-50/50 border-rose-200" : "bg-[#F7F8F3] border-[#D6DED2]"}`}>
                              <div className="flex items-center gap-2.5 min-w-0"><div className={`p-2 rounded-xl shrink-0 ${docStatus === "verified" ? "bg-emerald-100 text-emerald-800" : docStatus === "rejected" ? "bg-rose-100 text-rose-800" : "bg-amber-100 text-amber-800"}`}><FileText className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="font-extrabold text-[#243028] truncate text-xs">BAST Konsumen Terunggah</p><p className="text-[10px] text-[#66736A] font-mono truncate max-w-[160px] mt-0.5">{customerBast.fileName}</p></div></div>
                              <div className="flex flex-col gap-1.5 shrink-0 ml-2"><a href={customerBast.fileUrl} target="_blank" rel="noopener noreferrer" className="bg-[#4F6F52] hover:bg-[#3D563F] text-white font-extrabold text-[10px] px-2.5 py-1.5 rounded-lg shadow-sm transition-all text-center flex items-center justify-center gap-1 hover:scale-[1.02]"><ExternalLink className="h-3 w-3 text-white" />Unduh</a>
                        ...
                        <div className="pt-1.5 flex gap-2"><a href={`/marketing/bookings/${customerBast.bookingId}/bast/print`} target="_blank" rel="noopener noreferrer" className="w-full inline-flex items-center justify-center bg-white border border-[#D6DED2] text-[#4F6F52] hover:bg-[#F7F8F3]/50 font-extrabold text-xs px-3.5 py-2 rounded-xl shadow-sm transition-all gap-1.5"><FileText className="h-4 w-4" />Cetak Berita Acara BAST</a></div>
                      </div>
                    );
                  })()}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4"><h4 className="font-semibold text-sm text-foreground flex items-center gap-1.5"><Calendar className="h-4 w-4 text-primary" />{t("production.handover_est_title")}</h4>
                    <div className="space-y-3">{handoverEstimations.length === 0 ? <div className="text-center py-6 border border-dashed border-[#8FAF9A]/30 rounded-lg text-xs text-muted-foreground">{t("production.handover_empty")}</div> : handoverEstimations.map((est) => <div key={est.id} className="bg-background p-4 rounded-xl border border-[#8FAF9A]/30 shadow-sm space-y-3"><div className="flex items-center justify-between text-xs font-semibold"><span className="text-muted-foreground flex items-center gap-1">{t("production.handover_target_lbl")}:</span><Badge className="bg-[#DDE8D8] text-[#4F6F52] hover:bg-[#DDE8D8] font-bold shadow-none">{new Date(est.estimatedHandoverDate).toLocaleDateString("id-ID", { day: 'numeric', month: 'long', year: 'numeric' })}</Badge></div>{activeUnitBast ? <div className="flex items-center gap-1.5 text-[10px] font-black text-[#4F6F52] bg-[#DDE8D8]/50 px-2.5 py-1 rounded-lg border border-[#8FAF9A]/30 w-fit"><CheckCircle2 className="h-3 w-3 text-[#4F6F52]" />Sudah Selesai Serah Terima (BAST Aktif)</div> : <div className="flex items-center gap-1.5 text-[10px] font-black text-amber-800 bg-amber-50 px-2.5 py-1 rounded-lg border border-amber-200/50 w-fit"><Clock className="h-3 w-3 text-amber-600 animate-pulse" />Dalam Proses / Estimasi Konstruksi</div>}<div className="p-2.5 rounded bg-[#8FAF9A]/5 border border-[#8FAF9A]/10 text-xs text-foreground italic leading-relaxed">&ldquo;{est.calculationNote}&rdquo;</div><div className="text-[10px] text-muted-foreground text-right font-medium">{t("production.handover_calc_date")} {new Date(est.createdAt).toLocaleDateString()}</div></div>)}</div>
                  </div>
                </div>

                {/* Photo Gallery */}
                <div className="pt-6 border-t border-border space-y-3 mt-6">
                  <h4 className="font-semibold text-sm text-foreground flex items-center gap-1.5"><Camera className="h-4 w-4 text-[#4F6F52]" />Galeri Bukti Foto Fisik Lapangan</h4>
                  {(() => {
      ...
      <Dialog open={progressOpen} onOpenChange={(open) => { setProgressOpen(open); if (!open) { setUploadedPhotos([]); setSelectedFiles([]); } }}>
        <DialogContent className="sm:max-w-xl rounded-3xl bg-white/98 backdrop-blur-md border border-[#D6DED2] shadow-[0_8px_30px_rgb(143,175,154,0.15)] p-0 overflow-hidden font-sans">
          ...
          <Tabs value={progressTab} onValueChange={setProgressTab} className="w-full">
            <div className="px-6 pt-3 border-b border-border bg-[#F7F8F3]/50"><TabsList className="grid grid-cols-2 w-full h-9 bg-muted/60 p-0.5 rounded-lg border border-[#D6DED2]"><TabsTrigger value="form" className="text-xs font-semibold rounded-md py-1.5 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm">{t("production.btn_input_progress") || "Catat Progress"}</TabsTrigger><TabsTrigger value="history" className="text-xs font-semibold rounded-md py-1.5 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm">Riwayat & Galeri Foto</TabsTrigger></TabsList></div>
            <TabsContent value="form" className="m-0 focus-visible:outline-none">
              {(() => { const currentSpk = spks.find(s => s.id === newProgress.spkId); if (!currentSpk) return null; return <div className="mx-6 mt-4 p-4 bg-gradient-to-r from-[#DDE8D8]/60 via-white/80 to-[#DDE8D8]/30 border border-[#D6DED2] rounded-2xl flex items-center justify-between text-xs shadow-sm animate-scale-in"><div className="space-y-1"><p className="text-[10px] font-bold text-[#66736A] uppercase tracking-wider">Nomor SPK Kerja</p><p className="font-mono font-bold text-[#4F6F52] text-sm">{currentSpk.spkNumber}</p><p className="text-[10px] text-muted-foreground font-medium">Vendor: {currentSpk.vendorName || "Kontraktor Utama"}</p></div><div className="text-right space-y-1"><p className="text-[10px] font-bold text-[#66736A] uppercase tracking-wider">Unit / Kavling</p><p className="font-black text-[#243028] text-sm">{currentSpk.projectName} &bull; Kav. {currentSpk.unitCode}</p><p className="text-[10px] text-muted-foreground font-medium">Status SPK: <span className="capitalize font-bold text-amber-600">{SPK_STATUS_LABELS[currentSpk.status] || currentSpk.status.replace("_", " ")}</span></p></div></div>; })()}
              <form onSubmit={handleInputProgress} className="p-6 space-y-5 pt-4 max-h-[60vh] overflow-y-auto">
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5"><label className="font-bold text-[#243028] text-xs flex items-center gap-1.5"><ClipboardList className="h-4 w-4 text-[#8FAF9A]" />{t("production.progress_lbl_component") || "Komponen Item Pekerjaan"}</label>
                    <Select value={newProgress.workItemId} onValueChange={(val: string | null) => setNewProgress(prev => ({ ...prev, workItemId: val || "" }))} required items={currentSpkComponents.map(item => ({ label: `${item.name} — Bobot ${item.weightPct}% (Progres: ${item.currentProgress}%)`, value: item.id }))}>
                      ...
                      <SelectContent className="border-[#D6DED2] rounded-xl bg-white/95 backdrop-blur-md">{currentSpkComponents.map(item => <SelectItem key={item.id} value={item.id} className="text-xs font-semibold">{item.name} &mdash; Bobot {item.weightPct}% (Progres: {item.currentProgress}%)</SelectItem>)}</SelectContent>
                    </Select>
                  </div>

                  {newProgress.workItemId && <div className="p-4 bg-gradient-to-br from-[#8FAF9A]/5 via-white/40 to-[#8FAF9A]/10 border border-[#8FAF9A]/20 rounded-2xl space-y-3 text-xs shadow-sm animate-scale-in"><div className="flex justify-between items-center font-bold text-foreground"><span className="text-[#66736A] font-bold">Status Kemajuan Fisik:</span><span className={`font-black text-xs px-2.5 py-0.5 rounded-full ${isOverLimit ? "bg-rose-50 text-rose-600 border border-rose-200 animate-pulse" : "bg-[#DDE8D8] text-[#4F6F52] border border-[#8FAF9A]/25"}`}>{isOverLimit ? `⚠️ Melebihi Batas! (${currentProgressPct}% + ${newProgress.percentageAdded}% = ${currentProgressPct + newProgress.percentageAdded}%)` : `${currentProgressPct}% → ${newTotalProgress}%`}</span></div><div className="relative w-full h-3 bg-slate-100 rounded-full overflow-hidden flex border border-[#D6DED2]/40 shadow-inner"><div className="h-full bg-gradient-to-r from-[#4F6F52] to-[#608764] transition-all duration-500 rounded-l-full" style={{ width: `${currentProgressPct}%` }} /><div className={`h-full transition-all duration-500 ${isOverLimit ? "bg-red-400 animate-pulse" : "bg-gradient-to-r from-[#8FAF9A] to-[#A3C1AD]"} ${currentProgressPct === 0 ? "rounded-l-full" : ""}`} style={{ width: `${isOverLimit ? 100 - currentProgressPct : newProgress.percentageAdded}%` }} /></div></div>}
                  ...
                  {currentProgressPct === 100 ? <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-start gap-3 text-xs text-emerald-800 shadow-sm animate-scale-in"><CheckCircle2 className="h-5 w-5 text-emerald-600 shrink-0 mt-0.5" /><div className="space-y-1"><p className="font-bold">Item Pekerjaan Selesai (100%)</p><p className="text-emerald-700/90 font-medium">Komponen pekerjaan ini telah mencapai progress fisik 100%.</p></div></div> : (
                    <div className="space-y-4">
                      <div className="p-4 bg-white/80 backdrop-blur-sm border border-[#D6DED2] rounded-2xl shadow-sm space-y-3.5">
                        <div className="flex items-center justify-between text-xs font-bold text-[#243028]"><span className="flex items-center gap-1.5"><TrendingUp className="h-4 w-4 text-[#8FAF9A]" />{t("production.progress_lbl_pct") || "Tambahan Kemajuan Fisik"}</span><div className="flex items-baseline gap-1.5">{newProgress.workItemId && componentWeightPct > 0 && <span className="text-[10px] text-[#66736A] font-semibold">(Dampak Unit: +{((newProgress.percentageAdded || 0) * componentWeightPct / 100).toFixed(1)}%)</span>}<span className="text-[#4F6F52] font-black text-base tracking-tight">+{newProgress.percentageAdded}%</span></div></div>
                        ...
                        <div className="flex items-center justify-between gap-2 pt-1 flex-wrap">
                          <div className="flex gap-1.5">{[10, 25, 50].map((preset) => { const disabled = preset > (100 - currentProgressPct); return <Button key={preset} type="button" variant="outline" disabled={disabled} className={`text-[10px] font-bold px-3 py-1 h-7 rounded-full transition-all duration-200 hover:scale-105 active:scale-95 disabled:opacity-30 disabled:pointer-events-none ${newProgress.percentageAdded === preset ? "bg-[#4F6F52] text-white border-[#4F6F52] shadow-sm" : "border-[#D6DED2] text-[#4F6F52] hover:bg-[#8FAF9A]/10 hover:border-[#8FAF9A]/40 bg-white"}`} onClick={() => setNewProgress(prev => ({ ...prev, percentageAdded: preset }))}>+{preset}%</Button>; })}<Button type="button" variant="outline" className={`text-[10px] font-black px-3.5 py-1 h-7 rounded-full ${newProgress.percentageAdded === Math.max(1, 100 - currentProgressPct) ? "bg-[#4F6F52] text-white border-[#4F6F52] shadow-sm" : "border-[#4F6F52]/50 text-[#4F6F52] hover:bg-[#4F6F52]/10 bg-white"}`} onClick={() => setNewProgress(prev => ({ ...prev, percentageAdded: Math.max(1, 100 - currentProgressPct) }))}>Set 100%</Button></div>
                        </div>
                      </div>
                      <div className="space-y-1.5"><label className="font-bold text-[#243028] text-xs flex items-center gap-1.5"><Calendar className="h-4 w-4 text-[#8FAF9A]" />{t("production.progress_lbl_date") || "Tanggal Laporan Lapangan"}</label><Input type="date" required className="border-[#D6DED2] focus-visible:ring-2 focus-visible:ring-[#4F6F52]/20 h-10 text-xs rounded-xl bg-white/80 font-medium" value={newProgress.progressDate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewProgress(prev => ({ ...prev, progressDate: e.target.value }))} /></div>
                      <div className="space-y-1.5"><label className="font-bold text-[#243028] text-xs flex items-center gap-1.5"><Camera className="h-4 w-4 text-[#8FAF9A]" />Foto Dokumentasi</label>
                        <div onClick={() => document.getElementById('progress-photo-upload-tab')?.click()} className="border-2 border-dashed border-[#8FAF9A]/40 hover:border-[#4F6F52]/60 bg-[#F7F8F3]/40 hover:bg-[#8FAF9A]/5 rounded-2xl p-6 text-center cursor-pointer transition-all duration-200 group"><input id="progress-photo-upload-tab" type="file" accept="image/*" multiple className="hidden" onChange={(e) => { if (e.target.files) { const filesArray = Array.from(e.target.files); setSelectedFiles(prev => [...prev, ...filesArray]); setUploadedPhotos(prev => [...prev, ...filesArray.map(file => URL.createObjectURL(file))]); } }} /><div className="flex flex-col items-center justify-center space-y-2"><div className="p-2.5 bg-white rounded-full shadow-md text-[#4F6F52] group-hover:scale-110 transition-transform duration-300 border border-[#D6DED2]"><Plus className="h-4 w-4" /></div><span className="text-xs font-bold text-[#243028]">Klik atau seret foto ke sini</span><span className="text-[10px] text-slate-500 font-medium">Maks 4 foto, JPG/PNG/WebP, max 5MB</span></div></div>
                        {uploadedPhotos.length > 0 && <div className="grid grid-cols-4 gap-3.5 pt-2">{uploadedPhotos.map((photo, index) => <div key={index} className="relative group aspect-square rounded-xl overflow-hidden border border-[#8FAF9A]/30 shadow-sm animate-scale-in"><Image src={photo} alt={`Preview ${index}`} fill className="object-cover" /><button type="button" onClick={(ev) => { ev.stopPropagation(); setSelectedFiles(prev => prev.filter((_, i) => i !== index)); setUploadedPhotos(prev => prev.filter((_, i) => i !== index)); }} className="absolute top-1 right-1 p-1 bg-black/60 hover:bg-rose-600 rounded-full text-white transition-all duration-200 hover:scale-110 shadow-sm"><XCircle className="h-4 w-4" /></button></div>)}</div>}
                      </div>
                      <div className="space-y-1.5"><label className="font-bold text-[#243028] text-xs flex items-center gap-1.5"><MessageSquare className="h-4 w-4 text-[#8FAF9A]" />Catatan Lapangan (Opsional)</label><Textarea placeholder="Contoh: Pemasangan plafon gypsum tuntas 100%..." className="border-[#D6DED2] focus-visible:ring-2 focus-visible:ring-[#4F6F52]/20 text-xs rounded-xl min-h-[80px] bg-white/80" value={newProgress.notes} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNewProgress(prev => ({ ...prev, notes: e.target.value }))} /></div>
                    </div>
                  )}
                </div>
                <DialogFooter className="pt-3 border-t border-[#D6DED2]/40 mt-4"><Button type="button" variant="ghost" onClick={() => setProgressOpen(false)} className="text-xs text-slate-500 hover:text-slate-800 rounded-xl hover:bg-slate-50 h-10 px-4 font-bold">{t("production.btn_cancel") || "Batal"}</Button><Button type="submit" disabled={isSubmitting || isOverLimit || !newProgress.workItemId} className="bg-[#4F6F52] hover:bg-[#3D563F] text-white font-bold text-xs rounded-xl shadow-[0_4px_12px_rgba(79,111,82,0.2)] hover:scale-[1.02] active:scale-[0.98] transition-all h-10 px-5 flex items-center gap-1.5">{isSubmitting ? <><Loader2 className="h-3.5 w-3.5 animate-spin" />Menyimpan...</> : (t("production.btn_save_progress") || "Simpan Progres")}</Button></DialogFooter>
              </form>
            </TabsContent>
            ...
            <TabsContent value="history" className="m-0 focus-visible:outline-none p-6 pt-4 max-h-[60vh] overflow-y-auto space-y-4">
              ...
              {spkLogs && spkLogs.length > 0 ? (() => {
                ...
                if (filteredLogs.length === 0) return <div className="text-center py-10 border border-dashed border-[#8FAF9A]/30 rounded-2xl text-xs text-muted-foreground">Belum ada riwayat progres untuk komponen yang dipilih.</div>;
                return <div className="space-y-3 pt-1">{filteredLogs.map((item: any) => <div key={item.log.id} className="p-3.5 bg-[#8FAF9A]/5 border border-[#8FAF9A]/20 rounded-xl space-y-2 text-xs"><div className="flex justify-between items-center font-bold text-foreground"><span className="text-[#4F6F52]">{item.workItem?.name || "Komponen Pekerjaan"}</span><div className="flex items-center gap-2"><Badge className="bg-[#DDE8D8] text-[#4F6F52] font-semibold border border-[#8FAF9A]/25 rounded-md hover:bg-[#DDE8D8]">+{item.log.percentageAdded}% &rarr; {item.log.currentTotalPct}%</Badge>{currentSpk && (currentSpk.status === "active" || currentSpk.status === "proses_konstruksi" || currentSpk.status === "overdue") && <button type="button" title="Hapus log progres" onClick={async () => { const confirmed = window.confirm(`Hapus log +${item.log.percentageAdded}% untuk "${item.workItem?.name}"?`); if (!confirmed) return; setIsSubmitting(true); try { const res = await deleteProgressLog(item.log.id); if (res.success) { setSuccessMessage("✓ Log progres berhasil dihapus."); if (currentSpk.unitId) await handleViewUnitProgress(currentSpk.unitId); } } catch (err: any) { setErrorMessage(err.message || "Gagal menghapus log."); } finally { setIsSubmitting(false); } }} disabled={isSubmitting} className="p-1 text-slate-400 hover:text-rose-600 transition-colors rounded-lg hover:bg-slate-100 disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" /></button>}</div></div><div className="text-muted-foreground leading-relaxed">{item.log.notes ? `"${item.log.notes}"` : <span className="italic">Tidak ada catatan.</span>}</div>{((item.attachments && item.attachments.length > 0) || (item.attachment && item.attachment.fileUrl)) && <div className="pt-1.5"><span className="text-[10px] font-bold text-[#66736A] uppercase tracking-wider block mb-1.5">Bukti Foto</span><div className="flex flex-wrap gap-2">{item.attachments && item.attachments.length > 0 ? item.attachments.map((att: any, idx: number) => <div key={att.id || idx} className="relative h-24 w-36 rounded-lg overflow-hidden border border-[#8FAF9A]/30 group shadow-sm bg-white cursor-zoom-in"><a href={att.fileUrl} target="_blank" rel="noopener noreferrer"><Image src={att.fileUrl} alt={`Bukti ${idx + 1}`} fill className="object-cover group-hover:scale-105 transition-transform duration-200" /></a></div>) : <div className="relative h-24 w-36 rounded-lg overflow-hidden border border-[#8FAF9A]/30 group shadow-sm bg-white cursor-zoom-in"><a href={item.attachment.fileUrl} target="_blank" rel="noopener noreferrer"><Image src={item.attachment.fileUrl} alt="Bukti Progress" fill className="object-cover group-hover:scale-105 transition-transform duration-200" /></a></div>}</div></div>}<div className="text-[10px] text-muted-foreground pt-1 text-right font-medium">Dicatat: {new Date(item.log.progressDate).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}</div></div>)}</div>;
              })() : <div className="text-center py-10 border border-dashed border-[#8FAF9A]/30 rounded-2xl text-xs text-muted-foreground">Belum ada log progres pembangunan untuk SPK ini.</div>}
            </TabsContent>
          </Tabs>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={handoverOpen} onOpenChange={setHandoverOpen}>
        <DialogContent className="sm:max-w-lg rounded-3xl bg-white/98 backdrop-blur-md border border-[#D6DED2] shadow-[0_8px_30px_rgb(143,175,154,0.15)] p-0 overflow-hidden font-sans">
          <div className="bg-gradient-to-r from-[#DDE8D8]/70 via-white/80 to-transparent p-6 border-b border-[#D6DED2]"><DialogHeader><DialogTitle className="text-primary font-bold text-lg flex items-center gap-2"><Calendar className="h-5 w-5" />{t("production.handover_form_title")}</DialogTitle><DialogDescription className="text-xs">{t("production.handover_form_desc")}</DialogDescription></DialogHeader></div>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="space-y-1"><label className="font-semibold text-foreground text-xs">Tipe Estimasi Serah Terima (BAST)</label>
                <Select value={newHandover.handoverType} onValueChange={(val: "vendor_to_developer" | "developer_to_customer" | null) => setNewHandover(prev => ({ ...prev, handoverType: val || "vendor_to_developer" }))} required items={[{ label: "BAST Vendor ke Developer (Fisik 100%)", value: "vendor_to_developer" }, { label: "BAST Developer ke Konsumen (Serah Kunci)", value: "developer_to_customer" }]}>
                  <SelectTrigger className="w-full h-10 px-3 rounded-xl border border-[#8FAF9A]/30 focus:ring-primary bg-white text-xs"><SelectValue placeholder="Pilih Tipe Serah Terima">{newHandover.handoverType === "vendor_to_developer" ? "BAST Vendor ke Developer (Fisik 100%)" : "BAST Developer ke Konsumen (Serah Kunci)"}</SelectValue></SelectTrigger>
      ...
      <Dialog open={bastDialogOpen} onOpenChange={setBastDialogOpen}>
        <DialogContent className="sm:max-w-md rounded-3xl bg-white/98 backdrop-blur-md border border-[#D6DED2] shadow-[0_8px_30px_rgb(143,175,154,0.15)] p-0 overflow-hidden font-sans">
          <div className="bg-gradient-to-r from-[#DDE8D8]/70 via-white/80 to-transparent p-6 border-b border-[#D6DED2]"><DialogHeader><DialogTitle className="text-primary font-bold text-lg flex items-center gap-2"><CheckCircle2 className="h-5 w-5" />Konfirmasi Selesai Pembangunan</DialogTitle><DialogDescription className="text-xs">Unggah dokumen BAST fisik untuk menyelesaikan unit.</DialogDescription></DialogHeader></div>
          <form onSubmit={handleCompleteConstructionWithBast} className="p-6 space-y-4 pt-4">
            {bastUnit && <div className="p-3.5 bg-[#8FAF9A]/5 border border-[#8FAF9A]/20 rounded-2xl space-y-2 text-xs font-semibold text-foreground"><div className="flex justify-between"><span className="text-muted-foreground">Kavling / Unit:</span><span className="font-mono text-primary font-bold">{bastUnit.code}</span></div>{bastSpk && <div className="flex justify-between border-t border-[#8FAF9A]/10 pt-1.5"><span className="text-muted-foreground">SPK Terkait:</span><span className="font-mono">{bastSpk.spkNumber}</span></div>}</div>}
            <div className="space-y-1.5"><label className="font-semibold text-foreground text-xs">Unggah File PDF BAST</label><div onClick={() => document.getElementById('bast-pdf-upload-tab')?.click()} className="border-2 border-dashed border-[#8FAF9A]/30 hover:border-primary/50 bg-[#F7F8F3]/60 hover:bg-[#8FAF9A]/5 rounded-2xl p-6 text-center cursor-pointer transition-all duration-150 group"><input id="bast-pdf-upload-tab" type="file" accept="application/pdf" className="hidden" required onChange={(e) => { if (e.target.files && e.target.files.length > 0) setBastPdfFile(e.target.files[0]); }} /><div className="flex flex-col items-center justify-center space-y-2"><div className="p-2.5 bg-white rounded-full shadow-sm text-primary group-hover:scale-110 transition-transform duration-200"><UploadCloud className="h-5 w-5" /></div><span className="text-xs font-bold text-foreground">{bastPdfFile ? bastPdfFile.name : "Pilih File PDF BAST"}</span><span className="text-[10px] text-muted-foreground">{bastPdfFile ? `Ukuran: ${(bastPdfFile.size / 1024 / 1024).toFixed(2)} MB` : "Format PDF (Maks. 10MB)"}</span></div></div></div>
            <DialogFooter className="pt-2 border-t border-[#8FAF9A]/10 mt-4"><Button type="button" variant="ghost" onClick={() => setBastDialogOpen(false)} className="text-xs text-muted-foreground hover:text-foreground rounded-xl">Batal</Button><Button type="submit" disabled={isSubmitting || !bastPdfFile} className="bg-[#4F6F52] hover:bg-[#3D563F] text-white font-extrabold text-xs rounded-xl shadow-sm px-4 flex items-center gap-1.5">{isSubmitting ? <span className="flex items-center gap-1"><span className="h-3 w-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />Memproses...</span> : <><CheckCircle2 className="h-4 w-4 text-white" />Selesai & Jadikan Ready Stock</>}</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
    </div>
