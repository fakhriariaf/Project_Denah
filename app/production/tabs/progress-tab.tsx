...
export function ProgressTab({
  ...
  const handoverValidationError = React.useMemo(() => {
    ...
    if (newHandover.handoverType === "vendor_to_developer") {
      ...
      if (statusStr === "construction_done" || statusStr === "sold" || statusStr === "menunggu_serah_terima" || statusStr === "handover_complete") return "âš ï¸ BAST Vendor ke Developer untuk unit ini sudah selesai dilakukan. Silakan pilih BAST Developer ke Konsumen.";
      if (selectedUnit.constructionProgress < 100) return "âš ï¸ BAST Vendor ke Developer hanya dapat dikalkulasikan jika progres pembangunan unit sudah mencapai 100%.";
    } else if (newHandover.handoverType === "developer_to_customer") {
      const statusStr = selectedUnit.status as string;
      if (statusStr === "available" || statusStr === "belum_siap") return "âš ï¸ Unit belum terbooking oleh konsumen aktif.";
      if (statusStr === "construction") return "âš ï¸ Pembangunan unit fisik harus diserahterimakan oleh Vendor terlebih dahulu (Status unit harus 'Selesai Bangun').";
    }
    return null;
  ...
  const handleCompleteConstructionWithBast = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bastUnit || !bastSpk || !bastPdfFile) { setErrorMessage("âš ï¸ Silakan pilih file PDF Berita Acara Serah Terima (BAST) terlebih dahulu."); return; }
                ...
                <div className="pb-4 border-b border-border flex justify-between items-center">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {unit.readyStockSource === "construction_flow" && <span className="bg-[#EAF2EC] text-primary border border-primary/30 text-[10px] font-extrabold px-2 py-0.5 rounded-lg">ðŸ—ï¸ Konstruksi ERP</span>}
                      {unit.readyStockSource === "legacy_ready_stock" && <span className="bg-[#F4F6F0] text-[#606C5A] border border-primary/20 text-[10px] font-extrabold px-2 py-0.5 rounded-lg">ðŸ  Legacy Ready Stock</span>}
                      {unit.readyStockSource === "manual_ready_stock" && <span className="bg-amber-50 text-amber-800 border border-amber-200 text-[10px] font-extrabold px-2 py-0.5 rounded-lg">âš™ï¸ Manual Ready</span>}
                      {unit.isReadyStock ? <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-extrabold px-2 py-0.5 rounded-lg">âœ“ Ready Stock</span> : <span className="bg-blue-50 text-blue-700 border border-blue-200 text-[10px] font-extrabold px-2 py-0.5 rounded-lg">Indent</span>}
                    </div>
                  </div>
              ...
              <form onSubmit={handleInputProgress} className="p-6 space-y-5 pt-4 max-h-[60vh] overflow-y-auto">
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5"><label className="font-bold text-foreground text-xs flex items-center gap-1.5"><ClipboardList className="h-4 w-4 text-primary/70" />{t("production.progress_lbl_component") || "Komponen Item Pekerjaan"}</label>
                    <Select value={newProgress.workItemId} onValueChange={(val: string | null) => setNewProgress(prev => ({ ...prev, workItemId: val || "" }))} required items={currentSpkComponents.map(item => ({ label: `${item.name} â€” Bobot ${item.weightPct}% (Progres: ${item.currentProgress}%)`, value: item.id }))}>
                      <SelectTrigger className="w-full h-11 border-border focus-visible:ring-2 focus-visible:ring-ring rounded-xl bg-white/80 backdrop-blur-sm text-xs font-semibold"><SelectValue placeholder={t("production.progress_lbl_component") || "Pilih komponen pekerjaan..."}>{newProgress.workItemId ? (() => { const item = currentSpkComponents.find(w => w.id === newProgress.workItemId); return item ? `${item.name} â€” Bobot ${item.weightPct}% (Progres: ${item.currentProgress}%)` : undefined; })() : undefined}</SelectValue></SelectTrigger>
                  ...
                  {newProgress.workItemId && <div className="p-4 bg-gradient-to-br from-[#8FAF9A]/5 via-white/40 to-[#8FAF9A]/10 border border-primary/20 rounded-2xl space-y-3 text-xs shadow-sm animate-scale-in"><div className="flex justify-between items-center font-bold text-foreground"><span className="text-muted-foreground font-bold">Status Kemajuan Fisik:</span><span className={`font-black text-xs px-2.5 py-0.5 rounded-full ${isOverLimit ? "bg-rose-50 text-rose-600 border border-rose-200 animate-pulse" : "bg-secondary text-primary border border-primary/50/25"}`}>{isOverLimit ? `âš ï¸ Melebihi Batas! (${currentProgressPct}% + ${newProgress.percentageAdded}% = ${currentProgressPct + newProgress.percentageAdded}%)` : `${currentProgressPct}% â†’ ${newTotalProgress}%`}</span></div><div className="relative w-full h-3 bg-slate-100 rounded-full overflow-hidden flex border border-border/40 shadow-inner"><div className="h-full bg-gradient-to-r from-[#4F6F52] to-[#608764] transition-all duration-500 rounded-l-full" style={{ width: `${currentProgressPct}%` }} /><div className={`h-full transition-all duration-500 ${isOverLimit ? "bg-red-400 animate-pulse" : "bg-gradient-to-r from-[#8FAF9A] to-[#A3C1AD]"} ${currentProgressPct === 0 ? "rounded-l-full" : ""}`} style={{ width: `${isOverLimit ? 100 - currentProgressPct : newProgress.percentageAdded}%` }} /></div></div>}
          ...
          <form onSubmit={handleCreateHandover} className="p-6 space-y-4 pt-4 max-h-[75vh] overflow-y-auto">
            <div className="space-y-3 text-sm">
              <div className="space-y-1"><label className="font-semibold text-foreground text-xs">{t("production.handover_lbl_unit")}</label>
                <Select value={newHandover.unitId} onValueChange={(val: string | null) => { const cleanVal = val || ""; const linkedSpk = spks.find(s => s.unitId === cleanVal && s.status !== "cancelled"); setNewHandover(prev => ({ ...prev, unitId: cleanVal, spkId: linkedSpk?.id || "" })); }} required items={units.map(u => ({ label: `${u.code} â€” Progres ${u.constructionProgress}%`, value: u.id }))}>
                  <SelectTrigger className="w-full h-10 px-3 rounded-xl border border-primary/30 focus:ring-primary bg-card text-xs"><SelectValue placeholder={t("production.handover_lbl_unit")}>{newHandover.unitId ? (() => { const u = units.find(unit => unit.id === newHandover.unitId); return u ? `${u.code} â€” Progres ${u.constructionProgress}%` : undefined; })() : undefined}</SelectValue></SelectTrigger>
      ...
      <Dialog open={bastDialogOpen} onOpenChange={setBastDialogOpen}>
        <DialogContent className="sm:max-w-md rounded-3xl bg-white/98 backdrop-blur-md border border-border shadow-[0_8px_30px_rgb(143,175,154,0.15)] p-0 overflow-hidden font-sans">
          ...
          <form onSubmit={handleCompleteConstructionWithBast} className="p-6 space-y-4 pt-4">
            ...
            <DialogFooter className="pt-2 border-t border-primary/50/10 mt-4"><Button type="button" variant="ghost" onClick={() => setBastDialogOpen(false)} className="text-xs text-muted-foreground hover:text-foreground rounded-xl">Batal</Button><Button type="submit" disabled={isSubmitting || !bastPdfFile} className="bg-primary hover:bg-primary/90 text-white font-extrabold text-xs rounded-xl shadow-sm px-4 flex items-center gap-1.5">{isSubmitting ? <span className="flex items-center gap-1"><span className="h-3 w-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />Memproses...</span> : <><CheckCircle2 className="h-4 w-4 text-white" />Selesai & Jadikan Ready Stock</>}</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
    </div>
