...
export function SpmbTab({ materialRequests, spks }: SpmbTabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {successMessage && <div className="flex items-center gap-3 p-4 rounded-lg bg-[#DDE8D8] border border-[#8FAF9A]/40 text-[#4F6F52] text-sm font-medium"><span>✓ {successMessage}</span></div>}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <Button onClick={() => { setMaterialOpen(true); setMaterialStep(1); setMaterialNecessity(50); }} className="bg-primary hover:bg-[#4F6F52] text-primary-foreground font-semibold text-xs">
        ...
        <Table>
          ...
          <TableBody>
            {materialRequests.length === 0 ? (
              <TableRow><TableCell colSpan={8} className="py-12 text-center"><div className="flex flex-col items-center gap-3"><div className="h-16 w-16 rounded-full bg-[#DDE8D8]/50 flex items-center justify-center mx-auto"><Plus className="h-8 w-8 text-[#4F6F52]" /></div><div><p className="font-semibold text-[#243028] text-sm">{t("production.material_empty")}</p><p className="text-xs text-[#66736A] mt-1">{t("production.material_empty_desc")}</p></div></div></TableCell></TableRow>
            ) : (
              materialRequests.map((m) => (
                <TableRow key={m.id} className="hover:bg-[#8FAF9A]/5 transition-colors duration-150">
                  ...
                  <TableCell className="text-right">{m.status === "draft" && <Button size="sm" onClick={() => handleSubmitMaterialToFinance(m.id)} className="bg-primary hover:bg-[#4F6F52] text-primary-foreground font-semibold text-xs h-8">{t("production.btn_submit_to_finance")}</Button>}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
          ...
          <form onSubmit={handleCreateMaterial} className="p-6 space-y-4 pt-4 max-h-[75vh] overflow-y-auto">
              ...
              <div className="space-y-4 py-2">
                <div className="space-y-1"><label className="font-semibold text-foreground text-xs">{t("production.material_lbl_spk")}</label>
                  <Select value={newMaterial.spkId} onValueChange={(val: string | null) => setNewMaterial(prev => ({ ...prev, spkId: val || "" }))} required items={spks.map(s => ({ label: `${s.spkNumber} — ${s.title} (${s.unitCode})`, value: s.id }))}>
                    <SelectTrigger className="border-[#8FAF9A]/30 focus:ring-primary rounded-xl"><SelectValue placeholder={t("production.material_lbl_spk")}>{newMaterial.spkId ? (() => { const s = spks.find(spk => spk.id === newMaterial.spkId); return s ? `${s.spkNumber} — ${s.title} (${s.unitCode})` : undefined; })() : undefined}</SelectValue></SelectTrigger>
                ...
                {newMaterial.spkId && (() => { const selSpk = spks.find(s => s.id === newMaterial.spkId); if (!selSpk) return null; return <div className="p-3.5 bg-[#8FAF9A]/5 border border-[#8FAF9A]/20 rounded-2xl space-y-2 text-xs animate-scale-in"><span className="text-[10px] font-bold text-primary uppercase tracking-wider">Detail SPK</span><div className="grid grid-cols-2 gap-x-4 gap-y-2 pt-1 font-semibold text-foreground"><div className="space-y-0.5"><span className="text-[10px] text-muted-foreground font-medium">{t("production.info_project")}</span><div>{selSpk.projectName}</div></div><div className="space-y-0.5"><span className="text-[10px] text-muted-foreground font-medium">{t("production.info_kavling")}</span><div className="font-mono text-primary font-bold">{selSpk.unitCode}</div></div></div></div>; })()}
                <div className="flex justify-end pt-2"><Button type="button" disabled={!newMaterial.spkId} onClick={() => setMaterialStep(2)} className="bg-primary hover:bg-[#4F6F52] text-white font-semibold text-xs rounded-xl shadow-sm h-9 px-4">{t("production.btn_next")}</Button></div>
              </div>
            )}
            ...
            {materialStep === 2 && (
              <div className="space-y-4 py-2">
                ...
                <div className="flex justify-between pt-2">
                  ...
                  <Button type="button" disabled={!newMaterial.description.trim()} onClick={() => setMaterialStep(3)} className="bg-primary hover:bg-[#4F6F52] text-white font-semibold text-xs rounded-xl shadow-sm h-9 px-4">{t("production.btn_next")}</Button>
                </div>
              </div>
            )}
            ...
            {materialStep === 3 && (
              <div className="space-y-4 py-2">
                ...
                {Number(newMaterial.estimatedAmount) > 0 && (() => { const selSpk = spks.find(s => s.id === newMaterial.spkId); return <div className="p-3.5 bg-[#DDE8D8]/20 border border-[#8FAF9A]/20 rounded-2xl space-y-2 text-xs animate-scale-in"><span className="text-[10px] font-bold text-primary uppercase tracking-wider">Konfirmasi</span><div className="space-y-1.5 font-semibold text-foreground"><div className="flex justify-between"><span className="text-muted-foreground font-medium">{t("production.material_est_cost_lbl")}</span><span className="font-mono text-sm font-bold text-[#4F6F52] tabular-nums">Rp {Number(newMaterial.estimatedAmount).toLocaleString()}</span></div>{selSpk && <div className="flex justify-between border-t border-[#8FAF9A]/10 pt-1 text-[10px]"><span className="text-muted-foreground font-medium">{t("production.material_spk_ceiling")}:</span><span className="font-mono text-foreground font-bold tabular-nums">Rp {selSpk.rabAmount.toLocaleString()}</span></div>}</div></div>; })()}
                <div className="flex justify-between pt-2">
                  ...
                  <Button type="submit" disabled={isSubmitting || !newMaterial.estimatedAmount} className="bg-primary hover:bg-[#4F6F52] text-primary-foreground font-semibold text-xs rounded-xl shadow-sm h-9 px-4">{t("production.btn_submit_material")}</Button>
                </div>
              </div>
            )}
          </form>
