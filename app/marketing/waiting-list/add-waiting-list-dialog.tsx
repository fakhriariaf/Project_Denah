...
export function AddToWaitingListDialog({ projects, customers }: Props) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      toast.success("Antrean pembeli (waiting list) berhasil disimpan!");
      setOpen(false);
      form.reset();
      router.refresh();
    } catch (err: unknown) {
        ...
        <DialogContent className="sm:max-w-lg rounded-3xl bg-white/98 backdrop-blur-md border border-border shadow-[0_8px_30px_rgb(143,175,154,0.15)] p-0 overflow-hidden font-sans">
          <div className="bg-gradient-to-r from-[#DDE8D8]/70 via-white/80 to-transparent p-6 border-b border-border">
            <DialogHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <DialogTitle className="text-xl font-black text-foreground tracking-tight">{t("waiting_dialog.add_title")}</DialogTitle>
                  <DialogDescription className="text-xs text-muted-foreground mt-1 leading-relaxed">{t("waiting_dialog.add_desc")}</DialogDescription>
                </div>
              </div>
            </DialogHeader>
          </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="p-6 space-y-4 pt-3 overflow-y-auto max-h-[75vh]">
            ...
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-foreground">{t("waiting_dialog.customer")} <span className="text-destructive">*</span></Label>
            ...
            {/* Proyek */}
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-foreground">{t("waiting_dialog.project")} <span className="text-destructive">*</span></Label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-foreground">{t("waiting_dialog.pref_type")} <span className="text-primary/70 font-normal">{t("waiting_dialog.optional")}</span></Label>
                <Input {...form.register("preferredType")} placeholder="Misal: Type 36, Type 45" className="border-border placeholder:text-muted-foreground/70" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-foreground">{t("waiting_dialog.priority")} <span className="text-primary/70 font-normal">{t("waiting_dialog.optional")}</span></Label>
                <Input type="number" {...form.register("priority")} min={1} max={999} className="border-border font-mono" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-foreground">{t("waiting_dialog.budget_min")} <span className="text-primary/70 font-normal">{t("waiting_dialog.optional")}</span></Label>
                <Input type="number" step="1000000" {...form.register("budgetMin")} placeholder="0" className="border-border font-mono placeholder:text-muted-foreground/70" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-foreground">{t("waiting_dialog.budget_max")} <span className="text-primary/70 font-normal">{t("waiting_dialog.optional")}</span></Label>
                <Input type="number" step="1000000" {...form.register("budgetMax")} placeholder="0" className="border-border font-mono placeholder:text-muted-foreground/70" />
              </div>
            </div>

            <DialogFooter className="pt-2 gap-2">
              <Button type="button" variant="outline" onClick={() => setOpen(false)} className="border-border">{t("waiting_dialog.cancel")}</Button>
