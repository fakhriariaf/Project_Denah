  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger nativeButton={true} render={
        <Button variant="outline" size="sm" className="border-rose-200 text-rose-600 hover:bg-rose-50 hover:border-rose-300 flex items-center gap-1.5 h-8 text-xs rounded-xl btn-premium">
      ...
      <DialogContent className="sm:max-w-md rounded-3xl bg-white/98 backdrop-blur-md border border-border shadow-[0_8px_30px_rgb(143,175,154,0.15)] p-0 overflow-hidden font-sans">
        ...
        <div className="bg-gradient-to-r from-rose-50/80 via-white/90 to-transparent p-5 border-b border-rose-100/60">
          <DialogHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-lg font-black text-foreground tracking-tight">{t("booking_form.cancel_title")}</DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="p-3.5 bg-rose-50/80 text-rose-800 rounded-xl text-xs border border-rose-100/70 space-y-1">
            <span className="font-bold text-rose-900">{t("booking_form.cancel_important")}</span> <Translate namespace="booking_form" translationKey="cancel_desc" values={{ unitCode: booking.unitCode }} components={{ unit: <span className="font-mono font-bold bg-card px-1.5 py-0.5 rounded-lg border border-rose-100 shadow-sm text-slate-700" />, status: <span className="font-bold text-emerald-700" /> }} />
          </div>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="reason" className="text-xs font-semibold text-foreground">{t("booking_form.reason_label")}</Label>
            <Translate namespace="booking_form" translationKey="reason_placeholder" render={(ph) => (
              <textarea
                ...
                placeholder={ph}
                className="flex min-h-[100px] w-full bg-card border-input rounded-xl text-xs focus-visible:ring-2 focus-visible:ring-ring px-3 py-2"
              />
            )} />
          </div>
          ...
          <DialogFooter className="pt-4 gap-2 border-t border-rose-100/60">
            ...
            <Button 
              type="submit" 
              disabled={isLoading} 
              className="bg-rose-600 hover:bg-rose-700 text-white shadow-[0_4px_14px_rgba(220,38,38,0.25)] btn-premium h-9 rounded-xl font-bold text-xs px-4"
            >
