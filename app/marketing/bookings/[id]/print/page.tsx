...
export default async function PrintBookingPage({ params }: PrintPageProps) {
  ...
  return (
    <div className="min-h-screen bg-muted/30/60 print:bg-card pb-12 font-sans text-foreground">
      ...
      {/* Invoice sheet view */}
      <div className="print-area max-w-4xl mx-auto mt-6 bg-card border border-border rounded-3xl shadow-sage p-8 md:p-12 transition-all duration-300">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-6 border-b-2 border-[#4F6F52]/30 gap-6">
          <div className="space-y-1 md:flex-1">
            <h2 className="text-xl font-extrabold text-primary tracking-wide print-text-dark">
              {companyName}
            </h2>
          ...
          <div className="bg-secondary/50 border border-primary/30 rounded-2xl px-4 py-2 text-right shrink-0">
            <span className="text-[10px] font-bold text-primary uppercase tracking-wider block">{t("booking.print_doc_num")}</span>
            <span className="font-mono text-sm font-bold text-foreground print-text-dark">{booking.bookingNumber || "—"}</span>
          </div>
        </div>

        {/* INVOICE TITLE */}
        <div className="text-center my-8">
          <h1 className="text-2xl font-black text-foreground uppercase tracking-wider print-text-dark">
            {t("booking.print_sttb_title")}
          </h1>
          <p className="text-xs text-muted-foreground font-medium mt-1 print-text-dark">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          ...
          <div className="border border-border rounded-2xl p-5 bg-muted/30/30">
            <h3 className="text-xs font-bold text-primary uppercase tracking-wider mb-3 border-b border-border/60 pb-1.5 flex items-center gap-1.5 print-text-dark">
              <Award className="h-4 w-4" /> {t("booking.print_cust_detail")}
            </h3>
            <table className="w-full text-xs space-y-2">
              <tbody>
                <tr className="align-top">
                  <td className="w-24 font-semibold text-muted-foreground py-1">{t("booking.print_name")}</td>
                  <td className="w-3 text-muted-foreground/70 py-1">:</td>
                  <td className="font-bold text-foreground py-1 print-text-dark">{booking.customerName || "—"}</td>
                </tr>
                <tr className="align-top">
                  <td className="font-semibold text-muted-foreground py-1">{t("booking.print_phone")}</td>
                  <td className="text-muted-foreground/70 py-1">:</td>
                  <td className="font-mono py-1 print-text-dark">{booking.customerPhone || "—"}</td>
                </tr>
                <tr className="align-top">
                  <td className="font-semibold text-muted-foreground py-1">{t("booking.print_address")}</td>
                  <td className="text-muted-foreground/70 py-1">:</td>
                  <td className="text-muted-foreground py-1 leading-relaxed print-text-dark">{booking.customerAddress || "—"}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="border border-border rounded-2xl p-5 bg-muted/30/30">
            ...
            <table className="w-full text-xs space-y-2">
              <tbody>
                <tr className="align-top">
                  <td className="w-24 font-semibold text-muted-foreground py-1">{t("booking.print_housing")}</td>
                  <td className="w-3 text-muted-foreground/70 py-1">:</td>
                  <td className="font-bold text-foreground py-1 print-text-dark">{booking.projectName || "—"}</td>
                </tr>
                <tr className="align-top">
                  <td className="font-semibold text-muted-foreground py-1">{t("booking.print_lot_code")}</td>
                  <td className="text-muted-foreground/70 py-1">:</td>
                  <td className="py-1">
                    <span className="font-mono font-bold bg-secondary/60 text-primary border border-primary/20 px-1.5 py-0.5 rounded text-[10px]">
                      {booking.unitCode || "—"}
                    </span>
                  </td>
                </tr>
                <tr className="align-top">
                  <td className="font-semibold text-muted-foreground py-1">{t("booking.print_cluster_type")}</td>
                  <td className="text-muted-foreground/70 py-1">:</td>
                  <td className="font-semibold text-muted-foreground py-1 print-text-dark">
                    {booking.cluster ? `${booking.cluster} / ` : ""} {booking.typeName || "—"}
                  </td>
                </tr>
                <tr className="align-top">
                  <td className="font-semibold text-muted-foreground py-1">{t("booking.print_lt_lb")}</td>
                  <td className="text-muted-foreground/70 py-1">:</td>
                  <td className="font-mono py-1 print-text-dark">{booking.landArea} m² / {booking.buildingArea} m²</td>
                </tr>
                <tr className="align-top">
                  <td className="font-semibold text-muted-foreground py-1">{t("booking.print_unit_price")}</td>
                  <td className="text-muted-foreground/70 py-1">:</td>
                  <td className="font-mono font-bold text-primary py-1 print-text-dark">{formatRupiah(booking.unitPrice)}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="border border-border rounded-2xl overflow-hidden mb-8">
          <div className="px-5 py-3 border-b border-border bg-muted/30/70">
            <h3 className="text-xs font-bold text-primary uppercase tracking-wider print-text-dark">
              {t("booking.print_paid_detail")}
            </h3>
          </div>
          <table className="w-full text-xs text-left border-collapse">
            <thead>
              <tr className="border-b border-border text-muted-foreground font-bold bg-muted/30/20">
            ...
            <tbody className="divide-y divide-[#D6DED2]/60">
              <tr>
                <td className="py-3 px-5 font-semibold text-foreground print-text-dark">{t("booking.print_bf")}</td>
                <td className="py-3 px-5 text-center uppercase font-bold text-muted-foreground">{booking.paymentScheme || "—"}</td>
                <td className="py-3 px-5 text-right font-mono font-bold text-primary tabular-nums">{formatRupiah(booking.bookingFee)}</td>
              </tr>
              <tr>
                <td className="py-3 px-5 font-semibold text-foreground print-text-dark">{t("booking.print_dp")}</td>
                <td className="py-3 px-5 text-center uppercase font-bold text-muted-foreground">{booking.paymentScheme || "—"}</td>
                <td className="py-3 px-5 text-right font-mono font-bold text-primary tabular-nums">{formatRupiah(booking.dpAmount)}</td>
              </tr>
              <tr className="bg-secondary/20 font-bold text-sm">
                <td colSpan={2} className="py-3.5 px-5 text-primary print-text-dark">{t("booking.print_total_collected")}</td>
                <td className="py-3.5 px-5 text-right font-mono text-primary tabular-nums text-base">{formatRupiah(totalPaid)}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="border border-border rounded-2xl p-5 bg-muted/30/20 text-[10px] text-muted-foreground space-y-2 mb-12 print-text-dark">
          <h4 className="font-bold text-foreground text-xs uppercase tracking-wider mb-1 print-text-dark">{t("booking.print_terms_title")}</h4>
        ...
        <div className="grid grid-cols-2 gap-12 text-center text-xs mt-16">
          <div className="space-y-16">
            <div className="space-y-1">
              <span className="text-muted-foreground block">{t("booking.print_sign_buyer")}</span>
              <span className="text-[10px] text-muted-foreground/70 italic">{t("booking.print_sign_note")}</span>
            </div>
            <div className="space-y-1">
              <div className="w-48 border-b border-dashed border-[#66736A] mx-auto" />
              <span className="font-bold text-foreground block print-text-dark">{booking.customerName || "—"}</span>
            </div>
          </div>

          <div className="space-y-16">
            <div className="space-y-1">
              <span className="text-muted-foreground block">{t("booking.print_sign_receiver")}</span>
              <span className="text-[10px] text-muted-foreground/70 italic">{t("booking.print_sign_note")}</span>
            </div>
            <div className="space-y-1">
              <div className="w-48 border-b border-dashed border-[#66736A] mx-auto" />
              <span className="font-bold text-foreground block print-text-dark">{booking.marketingName || "—"}</span>
            </div>
          </div>
        </div>
