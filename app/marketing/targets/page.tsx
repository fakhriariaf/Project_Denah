  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-[#DDE8D8]/70 via-white/95 to-[#DDE8D8]/40 border border-border shadow-sage p-6">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-foreground tracking-tight">{t("targets.title")}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-card border border-border rounded-2xl p-4 shadow-sage flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-sky-500/10 text-sky-600 flex items-center justify-center shrink-0">
            <Target className="h-5 w-5" />
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">{t("targets.kpi_unit")}</p>
            <h3 className="text-xl font-black font-mono text-foreground">{totalTargetUnits}</h3>
          </div>
        </div>
        <div className="bg-card border border-border rounded-2xl p-4 shadow-sage flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">{t("targets.kpi_achieved")}</p>
            <h3 className="text-xl font-black font-mono text-foreground">{totalAchievedUnits}</h3>
          </div>
        </div>
        <div className="bg-card border border-border rounded-2xl p-4 shadow-sage flex items-center gap-3">
          <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${achievementPct >= 80 ? "bg-emerald-500/10 text-emerald-600" : achievementPct >= 50 ? "bg-amber-500/10 text-amber-600" : "bg-rose-500/10 text-rose-500"}`}>
            {achievementPct >= 80 ? <TrendingUp className="h-5 w-5" /> : <TrendingDown className="h-5 w-5" />}
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">{t("targets.kpi_pct")}</p>
            <h3 className="text-xl font-black font-mono text-foreground">{achievementPct}%</h3>
          </div>
        </div>
        <div className="bg-card border border-border rounded-2xl p-4 shadow-sage flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">{t("targets.kpi_team")}</p>
            <h3 className="text-xl font-black font-mono text-foreground">{marketingUsers.length}</h3>
          </div>
        </div>
      </div>

      {/* Chart Card — Pencapaian per Marketing */}
      <Card className="rounded-2xl border border-border shadow-sage">
        <CardHeader className="border-b border-border bg-muted/30/70">
        ...
        <CardContent className="pt-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {monthlyProgress.map((m) => (
              <div key={m.month} className="flex flex-col gap-1.5 p-2.5 rounded-xl bg-muted/30 border border-border/60">
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sage">
        <div className="px-6 py-3.5 border-b border-border bg-muted/30/70">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t("targets.table_title", { year: currentYear.toString() })}</span>
            <span className="text-xs font-mono text-primary/70">{t("targets.entries", { count: targets.length })}</span>
          </div>
        </div>

        {targets.length === 0 ? (
          <div className="py-16 text-center">
            <div className="h-16 w-16 rounded-full bg-secondary/60 flex items-center justify-center mx-auto mb-4">
              <Target className="h-8 w-8 text-primary" />
            </div>
            <p className="font-bold text-foreground">{t("targets.empty")}</p>
            <p className="text-xs text-muted-foreground mt-1">{t("targets.empty_desc", { year: currentYear.toString() })}</p>
          </div>
        ) : (
              ...
              <thead>
                <tr className="border-b border-border bg-muted/30/50">
                  {[t("targets.col_marketing"), t("targets.col_project"), t("targets.col_period"), t("targets.col_target"), t("targets.col_achieved"), t("targets.col_pct"), t("targets.col_amount"), ""].map(h => (
                    <th key={h} className="py-3 px-4 text-xs font-bold text-muted-foreground uppercase tracking-wider whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {targets.map(target => {
                  ...
                  return (
                    <tr key={target.id} className="border-b border-border/60 hover:bg-muted/30/50 transition-colors">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          <div className="h-7 w-7 rounded-lg bg-sky-500/10 text-sky-600 flex items-center justify-center text-xs font-black">{target.marketingName?.[0]?.toUpperCase()}</div>
                          <span className="text-sm font-semibold text-foreground">{target.marketingName}</span>
                        </div>
                      </td>
                      <td className="py-3 px-4 text-xs text-muted-foreground">{target.projectName}</td>
                      <td className="py-3 px-4">
                      ...
                      <td className="py-3 px-4 text-sm font-bold font-mono text-foreground tabular-nums">{target.targetUnits}</td>
                      ...
                      <td className="py-3 px-4 text-xs font-mono text-muted-foreground">{fmtRp(target.targetAmount)}</td>
                      <td className="py-3 px-4">
