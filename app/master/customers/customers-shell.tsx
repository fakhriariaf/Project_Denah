      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-[#DDE8D8]/70 via-white/95 to-[#DDE8D8]/40 border border-[#D6DED2] shadow-sage p-6">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-2xl font-black text-[#243028] tracking-tight">{t("cust.title")}</h2>
              <p className="text-sm text-[#66736A] mt-0.5">{t("cust.subtitle")}</p>
            </div>
          </div>
          {isEditor && (
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  <Input
                    ...
                    className="h-8 max-w-[180px] text-xs bg-[#F7F8F3]/50 border-[#D6DED2] rounded-xl focus:bg-white transition-premium"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                  />
                  <div className="flex flex-wrap gap-1 bg-[#F7F8F3] p-1 rounded-xl border border-[#D6DED2]/50">
                    <button
                      onClick={() => setStatusFilter("")}
                      className={`px-2 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-premium cursor-pointer ${
                        statusFilter === "" ? "bg-[#4F6F52] text-white shadow-sm" : "text-[#66736A] hover:bg-white/60"
                      }`}
                    >
                      {t("cust.all")}
                    </button>
                    {Object.keys(STATUS_MAP).map((s) => (
                      <button
                        ...
                        className={`px-2 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-premium cursor-pointer ${
                          statusFilter === s ? "bg-[#4F6F52] text-white shadow-sm" : "text-[#66736A] hover:bg-white/60"
                        }`}
                      >
                        {STATUS_MAP[s]?.label}
                      </button>
            ...
            <CardContent className="p-0">
              {filteredCustomers.length === 0 ? (
                <div className="py-16 text-center flex flex-col items-center justify-center">
                  ...
                  <p className="font-bold text-[#243028] text-sm">{t("cust.not_found")}</p>
                  <p className="text-xs text-[#66736A] mt-1">{t("cust.not_found_desc")}</p>
                </div>
              ) : (
                  ...
                  <Table>
                    <TableHeader className="bg-muted/40">
                      <TableRow className="border-b border-[#D6DED2]/40 hover:bg-transparent">
                        <TableHead className="font-bold text-[#66736A] uppercase tracking-wider text-[11px] py-4 pl-6">{t("cust.col_name")}</TableHead>
                        <TableHead className="font-bold text-[#66736A] uppercase tracking-wider text-[11px] py-4">{t("cust.col_phone")}</TableHead>
                        <TableHead className="font-bold text-[#66736A] uppercase tracking-wider text-[11px] py-4">Unit/Kavling</TableHead>
                        <TableHead className="font-bold text-[#66736A] uppercase tracking-wider text-[11px] py-4">{t("cust.col_source")}</TableHead>
                        <TableHead className="font-bold text-[#66736A] uppercase tracking-wider text-[11px] py-4">{t("cust.col_status")}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {filteredCustomers.map((c) => {
                        ...
                        return (
                          ...
                          >
                            <TableCell className="text-sm font-semibold text-[#243028] py-4 pl-6">{c.name}</TableCell>
                            <TableCell className="font-mono text-xs text-[#4F6F52] py-4">{c.phone}</TableCell>
                            <TableCell className="text-xs font-bold text-amber-700 py-4 font-mono">{c.unitCode || "-"}</TableCell>
                            <TableCell className="text-xs text-[#66736A] py-4 capitalize">
        ...
        <div className="lg:col-span-1 lg:sticky lg:top-[84px] transition-premium">
          {selectedCustomer ? (
            <Card className="border-[#D6DED2] shadow-sage-lg bg-white rounded-3xl overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="bg-[#4F6F52] text-white p-6 relative overflow-hidden flex items-center gap-4">
                <div className="absolute top-[-30%] right-[-10%] w-[50%] h-[150%] rounded-full bg-white/5 blur-xl pointer-events-none" />
                <div className="w-14 h-14 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-xl font-black text-white shrink-0 shadow-inner">
                  {selectedCustomer.name.substring(0, 2).toUpperCase()}
                </div>
                <div className="space-y-1 relative z-10 flex-1 min-w-0">
                  <span className="text-[9px] font-bold tracking-wider uppercase bg-white/10 backdrop-blur-md px-2 py-0.5 rounded border border-white/10">
                    {t("cust.sheet_title")}
                  </span>
              ...
              <CardContent className="p-6 space-y-5">
                {selectedCustomer.unitCode && (
                  <div className="space-y-1.5 animate-in fade-in duration-200">
                    <span className="text-[10px] font-bold text-[#66736A] uppercase tracking-wider block">Kavling / Unit Terikat</span>
                    <div className="bg-[#4F6F52]/5 border border-[#4F6F52]/20 rounded-2xl p-3 flex items-center justify-between text-xs font-semibold">
                      <span className="text-[#243028]">Kode Unit:</span>
                      <span className="font-mono text-[#4F6F52] font-black text-sm bg-white px-2 py-0.5 rounded border border-[#D6DED2]">{selectedCustomer.unitCode}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-1.5">
                  <span className="text-[10px] font-bold text-[#66736A] uppercase tracking-wider block">{t("cust.nik_title")}</span>
                  <div className="relative overflow-hidden bg-gradient-to-br from-[#4F6F52]/5 to-[#8FAF9A]/5 border border-[#D6DED2] rounded-2xl p-4 shadow-sm">
                    {/* Tiny chip image decorator */}
                    <div className="w-8 h-6 bg-[#E9C46A]/60 border border-[#8A6D1D]/20 rounded-md mb-2 opacity-80" />
                    <p className="font-mono text-base tracking-widest text-[#243028] font-bold mb-1.5">
                      {formatNIK(selectedCustomer.nik)}
                    </p>
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#D6DED2]/40">
                      <span className="text-[9px] font-bold text-[#66736A] tracking-wider uppercase">{t("cust.nik_valid")}</span>
                ...
                <div className="grid grid-cols-1 gap-2 pt-1">
                  <div className="flex items-center justify-between p-2.5 rounded-xl border border-[#D6DED2]/30 bg-[#F7F8F3]/60 hover:bg-[#F7F8F3] transition-premium">
                    <div className="flex items-center gap-2 min-w-0">
                      <Phone className="w-4 h-4 text-[#4F6F52]" />
                      <span className="font-mono text-xs text-[#243028] font-bold truncate">{selectedCustomer.phone}</span>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="w-7 h-7 hover:bg-[#8FAF9A]/10 rounded-lg text-[#66736A] cursor-pointer"
                  ...
                  <div className="flex items-center p-2.5 rounded-xl border border-[#D6DED2]/30 bg-[#F7F8F3]/60 hover:bg-[#F7F8F3] transition-premium gap-2 min-w-0">
                    <Mail className="w-4 h-4 text-[#4F6F52]" />
                    <span className="text-xs text-[#243028] font-medium truncate">{selectedCustomer.email || t("cust.email_empty")}</span>
                  </div>
                </div>

                {/* Address & Source */}
                <div className="space-y-3 bg-[#F7F8F3]/40 border border-[#D6DED2]/30 rounded-2xl p-4">
                  <div className="space-y-1">
                    <span className="text-[9px] font-bold text-[#66736A] uppercase tracking-wider block">{t("cust.address_title")}</span>
                    <div className="flex items-start gap-1.5">
                      <MapPin className="w-4 h-4 text-[#66736A] shrink-0 mt-0.5" />
                      <p className="text-xs text-[#243028] leading-relaxed font-semibold">
                        {selectedCustomer.address || t("cust.address_empty")}
                      </p>
                    </div>
                  </div>

                  <div className="border-t border-[#D6DED2]/30 my-2 pt-2 flex justify-between items-center">
                    <div>
                      <span className="text-[9px] font-bold text-[#66736A] uppercase tracking-wider block">{t("cust.source_title")}</span>
                      <span className="text-xs text-[#4F6F52] font-extrabold capitalize mt-0.5 block">
                        {selectedCustomer.source ? selectedCustomer.source.replace("_", " ") : "-"}
                      </span>
                    </div>
                    <div>
                      <span className="text-[9px] font-bold text-[#66736A] uppercase tracking-wider block">{t("cust.registered_since")}</span>
                      <span className="text-xs font-mono text-[#66736A] mt-0.5 block">
            ...
            <Card className="border-dashed border-2 border-[#D6DED2] bg-[#F7F8F3]/50 rounded-3xl p-8 text-center min-h-[300px] flex flex-col justify-center items-center">
              ...
              <h4 className="text-sm font-bold text-[#243028] mb-1">{t("cust.select_title")}</h4>
